...
exports.getSuggestedFriendsForUser = function(req, res){
  ...
  users.ID = friendships.userID WHERE ((username <>'+ JSON.stringify(loggedUser) + ') AND ((friendID <> id) OR (friendID IS NULL))) ORDER BY users.username;';
  connection.query(queryStr, function(error, result){
    if (error){
      res.send(404, []);
      console.log(error.sqlMessage);
...
exports.getSuggestionsFromFriends = function(req, res) {
  ...
  var queryStr = 'SELECT users.ID AS userID,\
                     ...
                     suggestions.Id AS suggestionId, \
...
exports.addNewSuggestion = function (req, res) {
  ...
  connection.query(queryStr, function(error, result){
    if (error) {
      ...
      connection.query(`select id from suggestions where suggestionName='${suggestionName}'`, function(error, result){
        res.send(result);
      })
...
exports.getItineraries = function(req, res) {
  var username = req.params.username
  var query = `select u.id as userId, up.id as id, up.planName as name
                from users u join user_plan up on u.id = up.userId 
...
exports.getItinerary = function(req, res) {
  ...
  var username = req.params.username;
  var query = `select s.id as suggestionId, s.userID, u.username,
  s.destinationID, s.suggestionName, s.suggestionDescription, s.photoLink from suggestions s 
  join plans_suggestions ps on ps.suggestionId = s.id 
  join user_plan up on ps.planId = up.id 
...
exports.addItinerary = function(req, res) {
  console.log(req.body)
  var query = `insert into user_plan (userId, planName) values ('${req.body.userId}', '${req.body.name}');`
  connection.query(query, function(error, result){
    if ( error ) {
      console.log(error)
      res.send(404, error)
    }
    connection.query(`select ID, planName from user_plan where planName = '${req.body.name}';`, function(error, result) {
      res.send(200, result)
    })
...
exports.addSuggestionToItinerary = function(req, res) {
  var query = `insert into plans_suggestions (planId, suggestionId) values ('${req.body.planId}', '${req.body.suggestionId}');`
  connection.query(query, function(error, result){
    if ( error ) {
      console.log(error)
      res.send(404, error)
...
exports.getSuggestionsForUser = function(req, res) {
  var queryString = `select * from suggestions where userId = '${req.params.userId}'`
