...
var googleMapsClient = require('@google/maps').createClient({
  key: 'AIzaSyC1Hyk7sf7JVMSxLVd0VovwTEUI1tp6W7Q'
});
...
exports.getSuggestionsFromFriends = function(req, res) {
  ...
  var queryStr = 'SELECT users.ID AS userID,\
                     users.username AS username,\
                     FK_friendships_users.friendID AS friendID,\
                     FK_users_friendships.username AS friendName,\
                     FK_suggestions_friendships.suggestionName AS suggestionName,\
                     FK_suggestions_friendships.suggestionDescription AS suggestionDescription,\
                     FK_suggestions_friendships.destinationID AS destinationID,\
                     FK_suggestions_friendships.photoLink AS photoLink, \
                     FK_destinations_suggestions.destinationName AS destinationName\
                  FROM users AS users\
                       INNER JOIN friendships AS FK_friendships_users\
                         ON users.ID = FK_friendships_users.userID\
                       INNER JOIN users AS FK_users_friendships\
                         ON FK_friendships_users.friendID = FK_users_friendships.ID\
                       INNER JOIN suggestions AS FK_suggestions_friendships\
                         ON FK_friendships_users.friendID = FK_suggestions_friendships.userID\
                       INNER JOIN destinations AS FK_destinations_suggestions\
                         ON FK_suggestions_friendships.destinationID = FK_destinations_suggestions.ID\
                  WHERE (users.username = '+JSON.stringify(username)+') AND (FK_destinations_suggestions.destinationName = '+JSON.stringify(location)+');';
  connection.query(queryStr, function(error, result){
    if (error) {
      console.log(error.sqlMessage);
      res.send(error.sqlMessage);
