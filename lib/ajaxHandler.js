...
exports.getItineraries = function(username, callback) {
   axios.get(`/api/itineraries/${username}`)
   .then((res) => {
     callback(res.data)
   })
}

exports.getItinerary = function(id, username, callback) {
   axios.get(`/api/itinerary/${username}/${id}`)
   .then((res) => {
     callback(res.data)
   })
}

exports.postItinerary = function(userId, name, callback) {
  axios.post('/api/itineraries', {userId: userId, name: name})
  .then((res) => {
    callback(res);
  })
}

exports.addSuggestionToItinerary = function(planId, suggestionId, callback) {
  axios.put('api/itineraries', {planId, suggestionId})
  .then((res) => {
    console.log(res)
  })
}

exports.getSuggestionsForUser = function(userId, callback) {
  axios.get(`/api/userSuggestions/${userId}`)
  .then((res) => {
    callback(res);
  })
}